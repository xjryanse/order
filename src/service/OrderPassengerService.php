...
class OrderPassengerService {
    ...
    use \xjryanse\order\service\passenger\FieldTraits;
    use \xjryanse\order\service\passenger\TriggerTraits;
    // 20240528:排车逻辑
    use \xjryanse\order\service\passenger\ArrangeTraits;
     ...
     * 20230421：班线标签分页统计
     */
    public static function circuitBusTagStaticsPaginate($con = [], $order = '', $perPage = 10, $having = '', $field = "*", $withSum = false) {
        $fields[] = 'circuit_bus_id';
        $fields[] = 'tag';
        $fields[] = 'count(1) as number';
        $fields[] = 'sum(if(is_ticked,1,0)) as ticketSeats';
        $fields[] = 'sum(if(is_ticked,0,1)) as noTicketSeats';

        $con[] = ['is_ref', '=', 0];
        $list = self::where($con)->field(implode(',', $fields))->group('circuit_bus_id,tag')->order('tag')->select();
        $listsArr = $list ? $list->toArray() : [];

        $res = DataList::dataPackPaginate($listsArr, true, ['number', 'ticketSeats', 'noTicketSeats']);
    ...
    public function getRoute() {
        $info = $this->get();
        $fromStationName = StationService::getInstance($info['from_station_id'])->fStation();
        $toStationName = StationService::getInstance($info['to_station_id'])->fStation();
     ...
     * @return type
     */
    public static function orderAllPrizeN($orderId) {
        $passengerList = OrderService::getInstance($orderId)->objAttrsList('orderPassenger');    
        return array_sum(array_column($passengerList, 'prize'));
    }
